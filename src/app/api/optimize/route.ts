...
export async function POST(request: Request) {
  try {
    ...
    // Try multiple env var sources for Vercel compatibility
    const envKey = process.env.DEEPSEEK_API_KEY || (process as any).env?.DEEPSEEK_API_KEY || ""
    const key = apiKey || envKey

    if (!key) {
      return Response.json({
        error: "No DeepSeek API key. Add it in API Settings.",
        debug: { hasEnvVar: !!envKey, envKeys: Object.keys(process.env).filter(k => k.includes("DEEPSEEK") || k.includes("SYS_")) }
      }, { status: 400 })
    }
    ...
    try {
      ...
      if (match) {
        try { parsed = JSON.parse(match[0]) } catch {
          return Response.json({ error: "Failed to parse: " + jsonStr.slice(0, 200) }, { status: 502 })
        }
      } else {
        return Response.json({ error: "Invalid response: " + jsonStr.slice(0, 200) }, { status: 502 })
      }
    }
    ...
    return Response.json({
      ...
      score: {
        ...
        breakdown: parsed.scoreBreakdown || { specificity: 75, structure: 75, negativeQuality: 75, styleConsistency: 75 },
      },
